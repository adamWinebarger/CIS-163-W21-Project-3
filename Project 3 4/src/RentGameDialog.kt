...
class RentGameDialog(parent: JFrame?, private val game: Game) : JDialog(parent, true), ActionListener {
    ...
    var closeStatus: Int
        private set
    //Since variables defualt to public, this statement makes it so wae can only access the var
    //from outside the class, but not modify it
    ...
    override fun actionPerformed(e: ActionEvent) {
        ...
        if (button === okButton) {
            ...
            try {
                ...
                rentonTemp.time = d1
                game.rentedOn = rentonTemp
                val dueDateTemp = GregorianCalendar()
                d2 = df.parse(txtDateDueDate.text)
                dueDateTemp.time = d2
                game.dueBack = dueDateTemp
                game.nameOfRenter = txtRentedName.text
